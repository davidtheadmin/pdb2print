...
#: Shape of a hydrogen-bond strut, in multiples of its shaft radius.
#:
#: A plain cylinder meeting a ribbon leaves a hard crease all the way round the
#: joint — the strut arrives at full width and stops, so the eye reads it as a
#: rod pushed through a plate rather than as part of the same object.  It is
#: also the weakest possible root: a sharp internal corner is where a printed
#: part cracks.  Both are the same fix, which is the fillet every cast or
#: moulded part has at the base of a boss.
#:
#: ``_FLARE_LENGTH`` has to be longer than the strut is buried, or the whole
#: flare hides inside the ribbon and nothing shows.  A ribbon is one shaft
#: radius thick at the centre-line where a strut ends, so 1.6 leaves roughly
#: half the flare standing proud on a strut that meets the face square on, and
#: more than that on the majority that arrive at an angle.
_FLARE_RADIUS = 2.0
_FLARE_LENGTH = 1.6
#: Straight segments the flare curve is drawn with.  Three is enough for a
#: 0.8 mm curve; the cost of more is triangles in a boolean that runs a hundred
#: times per chain.
_FLARE_STEPS = 3


def flared_strut(a, b, radius: float, segments: int = _SEGMENTS,
                 flare: float = _FLARE_RADIUS,
                 flare_len: float = _FLARE_LENGTH) -> Manifold:
    """A strut from ``a`` to ``b`` that widens into a fillet at each end.

    One primitive, not three: the silhouette is drawn once as a 2D profile and
    revolved, so a filleted strut costs a boolean against the same single solid
    a bare cylinder would have been.  ``Manifold.revolve`` turns the
    cross-section's Y axis into the result's Z axis, which is exactly what
    :func:`_rot_z_to` then aims at ``b - a``.

    The flare is quadratic rather than a circular fillet.  A circular fillet is
    tangent to the shaft at exactly its own radius from the end, so making it
    reach further out of the ribbon means making it wider in the same breath —
    and the width is what has to stay bounded, because a root fatter than the
    ribbon is wide would break out through the ribbon's *edges*.  Easing the
    radius in as ``(1 - t)²`` decouples the two: how far the flare reaches and
    how much wider it gets are then separate numbers to turn.
    """
    a = np.asarray(a, float)
    b = np.asarray(b, float)
    length = float(np.linalg.norm(b - a))
    ...
    root = max(radius, radius * float(flare))
    reach = min(radius * float(flare_len), 0.5 * length)
    if reach < 1e-6 or root - radius < 1e-6:
        return _cylinder_between(a, b, radius, segments)

    steps = max(1, int(_FLARE_STEPS))
    grow = root - radius

    profile = [(0.0, 0.0)]
    for k in range(steps + 1):                       # flare at the ``a`` end
        t = k / steps
        profile.append((radius + grow * (1.0 - t) ** 2, t * reach))
    for k in range(steps, -1, -1):                   # flare at the ``b`` end
        t = k / steps
        profile.append((radius + grow * (1.0 - t) ** 2, length - t * reach))
    profile.append((0.0, length))

    # Consecutive duplicates make a degenerate polygon; ``reach == length / 2``
    # produces one where the two flares meet.
    pts = [profile[0]]
    for p in profile[1:]:
        if abs(p[0] - pts[-1][0]) > 1e-9 or abs(p[1] - pts[-1][1]) > 1e-9:
            pts.append(p)

    solid = Manifold.revolve(cross_section(pts), segments)
    transform = np.column_stack([_rot_z_to(b - a), a])
    return solid.transform(transform.tolist())
